...
class CController extends CBaseController
{
	...
	{
	  // allows prefix to url path e.g. localhost non-virtual root
		if(is_array($url))
		{
			$route=isset($url[0]) ? $url[0] : '';
			$url=$this->createUrl($route,array_splice($url,1));
		} else {
		  if (Yii::app()->params['base_root']<>'' and !stristr($url,Yii::app()->params['base_root']))
    	  $url=Yii::app()->params['base_root'].$url;		  
		}

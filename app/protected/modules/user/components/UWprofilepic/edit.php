...
<div id="<?php echo $id;?>_container" style="display:none;margin:10px 0px;">
    <div style="width:<?php echo $params['thumbW']; ?>px;height:<?php echo $params['thumbH']; ?>px;overflow:hidden;float:left;margin:0px 5px 0px 0px;border:1px solid #ccc;">
		 <?php
		 if (stristr($model->{$field->varname},'http')===false) {
   		echo CHtml::image(Yii::app()->request->baseUrl . "/" .  $model->{$field->varname},'',array('id'=>'preview')); 
		 } else {
   		echo CHtml::image($model->{$field->varname},'',array('id'=>'preview')); 		   
		 }
		 ?>
    </div>
...
$(function() {
	...
	var uploader = new qq.FileUploader({
		...
		label:'Upload Photo',  // <?php echo $field->varname; ?>   
